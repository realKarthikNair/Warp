...
impl ActionView {
    ...
    async fn transmit_receive(
        ...
        let request = spawn_async(wormhole::transfer::v1::request(
            connection,
            relay_hints,
            TRANSIT_ABILITIES,
            self.cancel_future(),
    ...
    async fn transmit_send(
        ...
        let (_file, path, filename) = self.prepare_and_open_file(&path).await?;
        ...
        self.imp().context.borrow_mut().file_path = Some(path.clone());

        let offer = wormhole::transfer::Offer::new_file_or_folder(
        ...
        cancelable_future(
            spawn_async(async move {
                Box::pin(wormhole::transfer::send(
                    connection,
                    relay_hints,
                    TRANSIT_ABILITIES,
                    offer,
